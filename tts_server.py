...
Runs as a standalone Flask server on port 8001
Uses project_viva env (Python 3.9 + CUDA)

Start with:
  set PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python
  D:\\Too\\ana\\envs\\project_viva\\python.exe tts_server.py
"""
...
def generate_speech():
    ...
    POST body: { "text": "Hello world", "exaggeration": 0.5, "cfg": 0.5 }
    Returns: { "success": true, "audio_url": "/api/tts/audio/filename.wav", "filename": "filename.wav" }
    """
    try:
        data = request.json
        text = data.get("text", "")
        ...
        # Truncate very long text (Chatterbox works best with shorter segments)
        if len(text) > 500:
            text = text[:500] + "..."
